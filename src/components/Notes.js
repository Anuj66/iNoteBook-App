...
const Notes = (props) => {
    ...
    useEffect(() => {
        if(localStorage.getItem('jwt_token')){
            ...
            navigator('/login')
            props.showAlert('Please Login to get Authorized', 'warning')
        }
    },[])
        ...
        <>
                ...
                <div className="modal-dialog">
                    <div className="modal-content">
                        ...
                        <div className="modal-footer">
                            ...
                            <button disabled={note.etitle.length < 3 || note.edescription.length < 5} type="button" className="btn btn-primary" onClick={handleEditNote}>Save changes</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className={"my-3 row"}>
                <div className={"container"}>
                    {notes.length === 0 && 'No notes found! Please add a note'}
                </div>
                {notes.map((note) => {
